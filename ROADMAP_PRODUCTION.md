...
| Routes `/super-admin/*` protégées seulement au niveau contrôleur (`requireRole()`), pas par un middleware de routing dédié | ✅ Traité : `Core/Middleware/RoleMiddleware.php` existait déjà (dispatché par `Router::applyMiddlewares()` via le tag `role:xxx`) mais n'était utilisé par AUCUNE route du projet — pas un nouveau middleware à construire, juste à brancher. Ajouté `'role:super_administrateur'` sur les 6 routes `/super-admin/*` + `/admin/application` dans `config/urls.php`, en défense en profondeur : les `requireRole()` existants dans `SuperAdminController` sont conservés tels quels, pas remplacés. Une future action qui oublierait l'appel contrôleur reste désormais bloquée par le routeur avant même l'instanciation du contrôleur. |
