...
<?php
$selectedTypes = $company['com_types_ids'] ?? $company['types_ids'] ?? $company['types_ids_csv'] ?? [];
if (!is_array($selectedTypes)) {
    $selectedTypes = preg_split('/\s*,\s*/', (string) $selectedTypes, -1, PREG_SPLIT_NO_EMPTY) ?: [];
}
if ($selectedTypes === [] && !empty($company['com_type_id'])) {
    $selectedTypes = [(int) $company['com_type_id']];
}
$selectedTypes = array_values(array_unique(array_map('intval', $selectedTypes)));
?>
...
<form method="POST" action="<?= $isEdit ? '/companies/' . (int) $company['com_id'] . '/update' : '/companies/store' ?>">
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            <label>Types</label>
            <select name="com_types_ids[]" class="form-control" multiple size="8" required>
            ...
            <small class="form-text text-muted">Une société peut cumuler plusieurs types.</small>
        </div>
        <div class="col-md-6 mb-3">
            <label>Holding</label>
