      ...
      <div class="col-md-4">
        ...
        <div class="card">
          ...
          <form method="post" action="/profile/password" id="password-form">
            ...
            <div class="card-body">
              <div class="form-group"><label>Mot de passe actuel</label><input class="form-control" type="password" name="current_password" required></div>
              <div class="form-group"><label>Nouveau mot de passe</label><input class="form-control" type="password" name="password" id="password-new" minlength="10" required></div>
              <div class="form-group">
                <label>Confirmation</label>
                <input class="form-control" type="password" name="password_confirm" id="password-confirm" minlength="10" required>
                <small class="form-text text-danger d-none" id="password-mismatch">Les deux mots de passe ne sont pas identiques.</small>
              </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
          ...
          var pwd = document.getElementById('password-new');
          var confirm = document.getElementById('password-confirm');
          var error = document.getElementById('password-mismatch');

          function checkMatch() {
            var mismatch = pwd.value !== '' && confirm.value !== '' && pwd.value !== confirm.value;
            error.classList.toggle('d-none', !mismatch);
          ...
          form.addEventListener('submit', function (e) {
            if (!checkMatch()) {
              e.preventDefault();
              if (window.Swal) {
                window.Swal.fire({ icon: 'warning', title: 'Les deux mots de passe ne sont pas identiques.' });
              }
              confirm.focus();
            }
          });
