...
class ContextController extends AjaxController
{
    ...
    public function setCompany(): void
    {
        ...
        $this->refreshAuthorizationContext($this->user['id'], $companyId);
    ...
    private function refreshAuthorizationContext(int $userId, int $companyId): void
    {
        $roles = $this->authModel->listerRoles($userId, $companyId);
        $roleCodes = $this->roleCodes($roles);
        $permissions = $this->authModel->listerPermissions($userId, $companyId);
        ...
        $_SESSION['user']['role_names'] = array_values(array_filter(array_map(
            static fn(array $role): string => (string) ($role['rol_nom'] ?? ''),
            $roles
        )));
        $_SESSION['user']['permissions'] = $permissions;
        $_SESSION['user']['level'] = $this->applicationLevel($roleCodes);
        $_SESSION['permissions'] = $permissions;
        $_SESSION['user_roles'] = $roleCodes;
        $_SESSION['user_permissions'] = $permissions;
        $_SESSION['user_level'] = $_SESSION['user']['level'];
