...
class Router
{
    ...
    public function dispatch(string $method, string $uri): void
    {
        ...
        foreach ($this->routes as $route) {
            ...
            if (!$this->moduleIsEnabled($route->controller)) {
                $this->sendNotFound('Module desactive.');
                return;
            }

            // Route trouvée → appliquer les middlewares dans l'ordre
            $this->applyMiddlewares($route);
            TenantEntitlementMiddleware::checkControllerAccess($route->controller);
    ...
    private function applyMiddlewares(Route $route): void
    {
        foreach ($route->middlewares as $middleware) {
            ...
            match ($name) {
                ...
                'rate-limit' => RateLimitMiddleware::check('route'),
    ...
    private function moduleIsEnabled(string $controller): bool
    {
        if (!defined('MODULES_ENABLED')) {
            return true;
        }

        $module = (string) strtok($controller, '\\');
        return !array_key_exists($module, MODULES_ENABLED) || MODULES_ENABLED[$module] === true;
    }
